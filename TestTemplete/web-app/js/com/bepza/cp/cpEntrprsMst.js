...
function excludeDetail(detailId) {
    ...
    } else if (detailId == 'excludeCpEntrprsRvisedPrdctDtl') {
        if (isExcludeCpEntrprsRvisedPrdctDtl == true) {
            $("#excludeCpEntrprsRvisedPrdctDtl").find('*').each(function () {
                $(this).prop('disabled', false);
            });
            $('#showHideCpEntrprsRvisedPrdctDtl').removeClass('fa-plus').addClass('fa-minus');
            $('#showHideCpEntrprsRvisedPrdctDtl').text(' Exclude');
            $('#excludeCpEntrprsRvisedPrdctDtl').show();
            $(this).parent().find('fa-minus').remove().add('fa-plus');
            isExcludeCpEntrprsRvisedPrdctDtl = false;
        } else {
            $("#excludeCpEntrprsRvisedPrdctDtl").find('*').each(function () {
                $(this).prop('disabled', true);
            });
            $('#showHideCpEntrprsRvisedPrdctDtl').removeClass('fa-minus').addClass('fa-plus');
            $('#showHideCpEntrprsRvisedPrdctDtl').text(' Include');
            $('#excludeCpEntrprsRvisedPrdctDtl').hide();
            $(this).parent().find('fa-plus').remove().add('fa-minus');
            isExcludeCpEntrprsRvisedPrdctDtl = true;
        }
    }
}
...
function refreshProduct(d, r, t) {
    ...
    if (t > 0 || t != null || t != "") {
        $.ajax({
            type: "POST",
            async: false,
            url: contextPath + "/CommonAjax/getProductListByProductTypeId",
            data: {productTypeId: t},
            success: function (response) {
                $('#' + d + '_' + r + '_' + 'productId').empty().append(response);
            },
